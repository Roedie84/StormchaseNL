...
class MeteoCoordinator(VerouderdMixin, LocationMixin, DataUpdateCoordinator[dict]):
    ...
    def _uur_waarde(payload: dict | None, veld: str, stempel: str, nu=None):
        """Waarde rond het huidige moment uit een los antwoord."""
        blok = (payload or {}).get("hourly") or {}
        tijden = blok.get("time") or []
        waarden = blok.get(veld) or []

        exact = op_stempel(tijden, waarden, stempel)
        if exact is not None:
            return exact

        if nu is None:
            return None
        return dichtstbijzijnde(tijden, waarden, nu, MARGE_UUR)
    ...
    def _kwartier_waarde(payload: dict | None, veld: str, nu):
        ...
        return dichtstbijzijnde(
            blok.get("time") or [], blok.get(veld) or [], nu, MARGE_KWARTIER
        )
    ...
    async def _extra_velden(self, latitude: float, longitude: float) -> dict:
        ...
        try:
            ...
            if stats is not None:
                stats.bronnen["icon_d2"].fout(err)

        # GFS voor de Lifted Index, via het eigen eindpunt. Dat is
        # betrouwbaarder dan hetzelfde veld bij het gedeelde eindpunt
        # opvragen met een modelkeuze erbij.
        try:
            async with async_timeout.timeout(20):
                antwoord = await self._session.get(
                    LI_URL, params={**basis, "hourly": "lifted_index"}
                )
                antwoord.raise_for_status()
                uit["gfs"] = await antwoord.json()
            if stats is not None:
                stats.bronnen["lifted_index"].succes()
        except (aiohttp.ClientError, TimeoutError, ValueError) as err:
            _LOGGER.debug("Lifted Index niet beschikbaar: %s", err)
            if stats is not None:
                stats.bronnen["lifted_index"].fout(err)

        # Dezelfde grootheid uit meerdere modellen, om de spreiding te zien
        try:
            async with async_timeout.timeout(25):
                antwoord = await self._session.get(
                    ...
                    params={
                        **basis,
                        "models": ",".join(ENSEMBLE_MODELLEN),
                        "hourly": "cape",
                    },
                )
                antwoord.raise_for_status()
                uit["ensemble"] = await antwoord.json()
            if stats is not None:
                stats.bronnen["ensemble"].succes()
        except (aiohttp.ClientError, TimeoutError, ValueError) as err:
            _LOGGER.debug("Modelvergelijking niet beschikbaar: %s", err)
            if stats is not None:
                stats.bronnen["ensemble"].fout(err)

        # Ensemble: hoeveel leden komen boven de drempel uit?
        for model in (ENSEMBLE_LOKAAL, ENSEMBLE_WERELD):
            try:
                async with async_timeout.timeout(25):
                    antwoord = await self._session.get(
                        ENSEMBLE_URL,
                        params={**basis, "models": model, "hourly": "cape"},
                    )
                    antwoord.raise_for_status()
                    payload = await antwoord.json()

                if (payload.get("hourly") or {}).get("time"):
                    uit["ensemble_leden"] = payload
                    uit["ensemble_model"] = model
                    if stats is not None:
                        stats.bronnen["ensemble_leden"].succes()
                    break
            except (aiohttp.ClientError, TimeoutError, ValueError) as err:
                _LOGGER.debug("Ensemble %s niet beschikbaar: %s", model, err)
                if stats is not None:
                    stats.bronnen["ensemble_leden"].fout(err)
    ...
    async def _async_update_data(self) -> dict:
        ...
        if not times:
            return self.val_terug(ValueError("Open-Meteo gaf geen uurdata terug"))
        ...
        if lpi is None:
            lpi = self._uur_waarde(d2, "lightning_potential", stamp, nu_lokaal)

        updraft = self._uur_waarde(d2, "updraft", stamp, nu_lokaal)
        wolkentop = self._uur_waarde(d2, "convective_cloud_top", stamp, nu_lokaal)
        wolkenbasis = self._uur_waarde(d2, "convective_cloud_base", stamp, nu_lokaal)

        # Kwartierwaarden gaan voor op uurwaarden waar ze bestaan
        # Per model de waarde bij het huidige uur. Open-Meteo hangt de
        # modelnaam achter het veld wanneer je er meerdere opvraagt.
        ensemble = extra.get("ensemble")
        per_model = {
            naam: self._uur_waarde(ensemble, f"cape_{naam}", stamp, nu_lokaal)
            for naam in ENSEMBLE_MODELLEN
        }
        spreiding = beoordeel(per_model)

        # Per ensemblelid de hoogste waarde over de komende twaalf uur. Een
        # momentopname zegt weinig over een dag: convectie piekt vaak maar
        # een paar uur.
        leden_payload = extra.get("ensemble_leden")
        blok = (leden_payload or {}).get("hourly") or {}
        pieken = []
        for sleutel, reeks in blok.items():
            if not sleutel.startswith("cape") or sleutel == "time":
                continue
            venster = [w for w in (reeks or [])[index : index + 12] if w is not None]
            if venster:
                pieken.append(max(venster))

        kansen = vat_ensemble_samen(pieken)
        kansen["model"] = extra.get("ensemble_model")
        ...
        if li is None:
            li = self._uur_waarde(gfs, "lifted_index", stamp, nu_lokaal)
        ...
        return self.onthoud({
            ...
            "spreiding": spreiding,
            "ensemble": kansen,
            "spreiding_tekst": samenvatting(spreiding),
            # Onderscheid tussen een model dat het veld niet levert en een
            # opzoekactie die ernaast greep
            "li_waarden_in_antwoord": aantal_gevuld(
                ((gfs or {}).get("hourly") or {}).get("lifted_index")
            ),
