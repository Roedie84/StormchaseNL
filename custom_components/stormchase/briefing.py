...
class Briefing:
    ...
    def _regel_onweer(self) -> str | None:
        ...
        if meteo is not None and meteo.data:
            ...
            # Zijn de modellen het oneens, dan hoort dat erbij: het getal
            # zegt dan minder dan het lijkt.
            spreiding = meteo.data.get("spreiding") or {}
            if spreiding.get("overeenstemming") == "modellen zijn verdeeld":
                regel += (
                    f" Let op: {meteo.data.get('spreiding_tekst', '')}, "
                    "de modellen zijn het oneens."
                )

            # De kans uit het ensemble zegt meer dan een enkele uitkomst
            kansen = meteo.data.get("ensemble") or {}
            if kansen.get("kans_onweer") is not None:
                regel += (
                    f" Ensemble: {kansen['kans_onweer']} procent kans op onweer"
                )
                if kansen.get("kans_zwaar"):
                    regel += f", {kansen['kans_zwaar']} procent op zwaar weer"
                regel += "."
