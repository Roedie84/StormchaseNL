...
class SpreadSensor(CoordinatorEntity[MeteoCoordinator], SensorEntity):
    """Hoe eens de weermodellen het met elkaar zijn over de energie.

    Een enkel getal leest als zekerheid. Zijn zes modellen het erover eens
    dat er 2000 J/kg zit, dan kun je daar een dag op plannen. Loopt de
    schatting van 200 tot 2600, dan zegt de mediaan weinig.
    """

    _attr_has_entity_name = True
    _attr_translation_key = "model_agreement"

    def __init__(self, coordinator: MeteoCoordinator, entry: ConfigEntry) -> None:
        """Initialiseer de sensor."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{entry.entry_id}_model_agreement"
        self._attr_device_info = _device(entry)

    @property
    def native_value(self) -> str | None:
        """Het oordeel over de overeenstemming."""
        if self.coordinator.data is None:
            return None
        return (self.coordinator.data.get("spreiding") or {}).get("overeenstemming")

    @property
    def extra_state_attributes(self) -> dict:
        """De cijfers achter het oordeel, per model."""
        data = self.coordinator.data or {}
        spreiding = data.get("spreiding") or {}
        return {
            "samenvatting": data.get("spreiding_tekst"),
            "mediaan": spreiding.get("mediaan"),
            "laagste": spreiding.get("laagste"),
            "hoogste": spreiding.get("hoogste"),
            "spreiding": spreiding.get("spreiding"),
            "aantal_modellen": spreiding.get("aantal_modellen"),
            "per_model": spreiding.get("modellen"),
        }


class MetingSensor(CoordinatorEntity[MetingCoordinator], SensorEntity):
    """De gemeten temperatuur op het dichtstbijzijnde weerstation.

    Alles wat de integratie verder toont is voorspeld. Deze waarde is
    gemeten, en het verschil met het model zegt of je de verwachting van
    vandaag kunt vertrouwen.
    """

    _attr_has_entity_name = True
    _attr_translation_key = "meting"
    _attr_native_unit_of_measurement = "\u00b0C"
    _attr_device_class = SensorDeviceClass.TEMPERATURE
    _attr_state_class = SensorStateClass.MEASUREMENT
    _attr_suggested_display_precision = 1

    def __init__(self, coordinator: MetingCoordinator, entry: ConfigEntry) -> None:
        """Initialiseer de sensor."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{entry.entry_id}_meting"
        self._attr_device_info = _device(entry)
    ...
    def native_value(self):
        ...
        if self.coordinator.data is None:
            return None
        return self.coordinator.data.get("temperatuur")

    @property
    def extra_state_attributes(self) -> dict:
        """De overige metingen en welk station ze leverde."""
        data = self.coordinator.data
        if not data:
            return {}
...
class BronstatusSensor(CoordinatorEntity[StormCoordinator], SensorEntity):
    ...
    Bij een storing blijven oude waarden staan in plaats van te verdwijnen.
    Zonder deze sensor zou je dat niet merken en naar verouderde cijfers
    ...
    def native_value(self) -> str:
        """Kort oordeel over de bronnen."""
        haperend = self._haperend()
        if not haperend:
            return "alles in orde"
        if len(haperend) == 1:
            return f"{haperend[0]} hapert"
        return f"{len(haperend)} bronnen haperen"

    @property
    def extra_state_attributes(self) -> dict:
        """Per bron het slaagpercentage en de laatste fout."""
        if self._stats is None:
            return {}
        return {
            "haperend": self._haperend(),
...
class EnsembleSensor(CoordinatorEntity[MeteoCoordinator], SensorEntity):
    """De kans op onweer, uit de leden van een ensemble.

    Een ensemble draait hetzelfde model meerdere keren met licht
    verschillende beginwaarden. Dat levert geen enkele uitkomst maar een
    kans: veertien van de twintig leden boven de drempel is zeventig procent.
    Een mediaan van 1200 J/kg ziet er goed uit, maar als veertig procent van
    de leden eronder zit weet je dat het alle kanten op kan.
    """

    _attr_has_entity_name = True
    _attr_translation_key = "ensemble"
    _attr_native_unit_of_measurement = "%"
    _attr_state_class = SensorStateClass.MEASUREMENT
    ...
    def __init__(self, coordinator: MeteoCoordinator, entry: ConfigEntry) -> None:
        """Initialiseer de sensor."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{entry.entry_id}_ensemble"
        self._attr_device_info = _device(entry)

    @property
    def native_value(self) -> int | None:
        """Het percentage leden dat onweer toelaat."""
        if self.coordinator.data is None:
            return None
        return (self.coordinator.data.get("ensemble") or {}).get("kans_onweer")

    @property
    def extra_state_attributes(self) -> dict:
        """De kans op zwaar weer, de spreiding tussen de leden en het model."""
        if self.coordinator.data is None:
            return {}
        return self.coordinator.data.get("ensemble") or {}
